...
def list_doctors(
    ...
    """List available doctors — restricted to Dr. Sam only."""
    filtered_doctors = [
        doc for doc in db.query(Doctor).filter(Doctor.is_available == True).all()
        if "sam" in doc.name.lower() or "luzulane" in doc.name.lower()
    ]
